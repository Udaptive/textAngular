						...
						if(!_isReadonly && !BLOCKED_KEYS.test(event.keyCode)){
							...
							if(_defaultVal !== '' && event.keyCode === 13){
								if(!event.shiftKey){
									...
									if(selection.tagName.toLowerCase() !== attrs.taDefaultWrap && selection.tagName.toLowerCase() !== 'li' && (selection.innerHTML.trim() === '' || selection.innerHTML.trim() === '<br>' || $(selection).text() === '' || $(selection).find('span').html() === '<br>')){
										var _new = angular.element(_defaultVal);
    ...
    return {
      ...
      link: function(scope, element, attrs, ngModel){
        ...
        if(attrs.taPaste){
          ...
          escope._pasteHandler = function(_html){
